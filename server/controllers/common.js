...
router.get(['/e', '/e/*'], async (req, res, next) => {
  ...
  if (WIKI.config.lang.namespacing && !pageArgs.explicitLocale) {
    return res.redirect(`/${WIKI.config.prefix}/e/${pageArgs.locale}/${pageArgs.path}`)
  }
...
router.get(['/h', '/h/*'], async (req, res, next) => {
  ...
  if (WIKI.config.lang.namespacing && !pageArgs.explicitLocale) {
    return res.redirect(`/${WIKI.config.prefix}/h/${pageArgs.locale}/${pageArgs.path}`)
  }
  ...
  if (page) {
    ...
    res.redirect(`/${WIKI.config.prefix}/${pageArgs.path}`)
  }
})
...
router.get(['/i', '/i/:id'], async (req, res, next) => {
  ...
  if (pageId <= 0) {
    return res.redirect('/${WIKI.config.prefix}/')
  }
  ...
  if (WIKI.config.lang.namespacing) {
    return res.redirect(`/${WIKI.config.prefix}/${page.localeCode}/${page.path}`)
  } else {
    return res.redirect(`/${WIKI.config.prefix}/${page.path}`)
  }
})
...
router.get(['/s', '/s/*'], async (req, res, next) => {
  ...
  if (WIKI.config.lang.namespacing && !pageArgs.explicitLocale) {
    return res.redirect(`/${WIKI.config.prefix}/s/${pageArgs.locale}/${pageArgs.path}`)
  }
...
router.get('/*', async (req, res, next) => {
  ...
  if (isPage) {
    if (WIKI.config.lang.namespacing && !pageArgs.explicitLocale) {
      return res.redirect(`/${WIKI.config.prefix}/${pageArgs.locale}/${pageArgs.path}`)
    }
    ...
    try {
      ...
      if (!effectivePermissions.pages.read) {
        ...
        if (pageArgs.path === 'home' && req.user.id === 2) {
          return res.redirect('/${WIKI.config.prefix}/login')
        }
        _.set(res.locals, 'pageMeta.title', 'Unauthorized')
